...
- `selfupdate.go` — `UpdateDeps` + `DefaultUpdateDeps` + `resolveDeps`, the `SelfUpdate` orchestration (resolve → compare → download → verify → extract → atomic replace), the `updateStatus` JSON shape + `emitStatus` (routes JSON through `cli.WriteJSON` for byte-compat), and `atomicReplace` (Unix rename-over; Windows rename-aside + rollback).
- `download.go` — HTTP helpers (`httpGet`, `downloadFile`, `downloadBody`) and `verifyChecksum` (parses goreleaser's `<hex>  <filename>` format and sha256s the archive).
