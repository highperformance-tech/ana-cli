...
- `create.go` — `newCreateGroup` (dialect-selector Group) + the shared `resolveSecret(flagName, val, stdin, r)` helper reused by every per-dialect secret-bearing leaf (password, OAuth client secret, PAT, …) and `update.go`. `flagName` parameterises the error messages so they read `--<name>-stdin set but stdin was empty`.
...
- `create_snowflake_oauth_sso.go` — `snowflakeOAuthSSOCmd` (leaf for `snowflake oauth-sso`; `authStrategy=oauth_sso`). Sends `oauthClientId` + `oauthClientSecret`; server accepts a pending-OAuth row, but the user must complete the handshake in a browser (CLI cannot receive the `/auth/snowflake/callback` redirect).
- `create_snowflake_oauth_individual.go` — `snowflakeOAuthIndividualCmd` (leaf for `snowflake oauth-individual`; `authStrategy=per_member_oauth`). Same wire fields as `oauth-sso`; no up-front handshake — each member authenticates lazily at first query.
...
- `create_snowflake_oauth_sso_test.go` — same shape; asserts `authStrategy=oauth_sso`, populated `oauthClientId`/`oauthClientSecret`, and absence of username/password/privateKey.
- `create_snowflake_oauth_individual_test.go` — same shape; asserts `authStrategy=per_member_oauth`.
